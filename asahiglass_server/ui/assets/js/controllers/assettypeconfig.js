...
/* Controllers */
angular.module('app')
    .controller('assettypeConfig', ['$scope', 'md5', '$rootScope', '$http', 'sessionInjector', 'Config', 'Sessioncheck', '$timeout', '$location', '$window', 'fileUpload', 'toaster',
        function ($scope, md5, $rootScope, $http, sessionInjector, config, Sessioncheck, $timeout, $location, $window, fileUpload, toaster) {
            ...
            $scope.Add = function () {
                ...
                for (var i = 0; i < $scope.Customers.length; i++) {
                    ...
                    if (angular.lowercase($scope.Customers[i].Name) == angular.lowercase($scope.Name)) {
                       // alert($scope.Name + ' attribute is already exists, Please choose different attribute name.')
                       toaster.pop({
                            type: 'warning',
                            body: 'Attribute is already exists, Please choose different attribute name.',
                            timeout: 3000
                        });                       
                        return;
                    }
                ...
                $http.get(config.host + '/assetType')
                    .then(function (res) {
                        if (res.data.success == true) {
                          ...
                          //  alert("Device data: " + JSON.stringify(assetList));
                            datatable(assetList);
                        }
                    });
            }
